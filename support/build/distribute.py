...
#
"""
Fork of the PyBuilder distutils_plugin
"""
import os
import string
import subprocess
...
from pybuilder.core   import Dependency
...
def initialize_distutils_plugin(project) :
    ...
    project.set_property_if_unset("distutils_devpi_user" , "root")
    project.set_property_if_unset("distutils_devpi_index", "dev")
...
def render_setup_script(project) :
    ...
    template_values = {
        ...
        "remove_hardlink_capabilities_for_shared_filesystems": (
            "import os\ndel os.link" if project.get_property("distutils_issue8876_workaround_enabled") else ""
            )
    }
...
def build_binary_distribution(project, logger) :
    ...
    run_setup_commands(project, logger, commands)


@task("develop")
@depends("package")
@description("Create an .egg-link in site-packages back to the packaged source")
def build_developer_distribution(project, logger) :
    logger.info("Creating .egg-link in site packages to %s", project.expand_path("$dir_dist"))
    run_setup_commands(project, logger, ["develop"])
...
def strip_comments(requirements) :
    return [requirement for requirement in requirements if not requirement.strip().startswith("#")]
...
def build_install_dependencies_string(project) :
    dependencies = [
        dependency for dependency in project.dependencies if isinstance(dependency, Dependency) and not dependency.url
        ]

    requirements = [
        requirements for requirements in project.dependencies if isinstance(requirements, RequirementsFile)
        ]
...
def build_dependency_links_string(project) :
    dependency_links = [
        dependency for dependency in project.dependencies if isinstance(dependency, Dependency) and dependency.url
        ]
